...
public class Pressure : IUnitValue
{
    public Pressure(double metricValue)
    {
    ...
    {
        var value = UnitsNet.Pressure.FromHectopascals(MetricValue);
        
        return Settings.Units == Units.Metric
            ? value.Hectopascals.ToString("N0")
            : value.InchesOfMercury.ToString("N1");
    }
