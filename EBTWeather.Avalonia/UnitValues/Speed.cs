...
public class Speed : IUnitValue
{
    public Speed(double metricValue)
    {
    ...
    {
        var value = UnitsNet.Speed.FromKilometersPerHour(MetricValue);
        
        return Settings.Units == Units.Metric
            ? value.KilometersPerHour.ToString("F0")
            : value.MilesPerHour.ToString("F0");
    }
